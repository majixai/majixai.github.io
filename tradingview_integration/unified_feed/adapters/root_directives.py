...
def _classify_kind(path: Path) -> str:
    if path.suffix.lower() in {".json", ".yaml", ".yml", ".csv", ".dat", ".db", ".gz", ".txt"}:
        return "data"
    return "code"
...
class DirectiveCatalog:
    ...
    def get_by_category(self, category: str) -> List[DirectiveEntry]:
        attr_map = {
            ...
            "math": "math_files",
            "infra": "infra_files",
            "root_python": "root_python_files",
        }
        return list(getattr(self, attr_map.get(category, f"{category}_files"), []))

    def has_category(self, category: str) -> bool:
        return bool(self.get_by_category(category))

    def get_by_source_dir(self, source_dir: str) -> List[DirectiveEntry]:
        return [entry for entry in self.entries if entry.source_dir == source_dir]

    def has_source_dir(self, source_dir: str) -> bool:
        return any(entry.source_dir == source_dir for entry in self.entries)


class RootDirectives:
    def __init__(self, root: Optional[Path] = None, deep: bool = True):
    ...
    def _collect_category(self, directory: Path, category: str) -> List[DirectiveEntry]:
    ...
    def scan(self) -> DirectiveCatalog:
        ...
        for dir_name in sorted(set(MATH_ROOT_DIRS)):
            entries = self._collect_category(self.root / dir_name, "math")
            catalog.math_files.extend(entries)
            catalog.entries.extend(entries)
            for entry in entries:
                catalog.dir_summary[entry.source_dir] = catalog.dir_summary.get(entry.source_dir, 0) + 1

        for dir_name in sorted(set(FINANCE_ROOT_DIRS)):
            entries = self._collect_category(self.root / dir_name, "finance")
            catalog.finance_files.extend(entries)
            catalog.entries.extend(entries)
            for entry in entries:
                catalog.dir_summary[entry.source_dir] = catalog.dir_summary.get(entry.source_dir, 0) + 1

        for dir_name in sorted(set(INFRA_ROOT_DIRS)):
            category = "infra"
            if dir_name == "dbs":
                category = "dbs"
            elif dir_name == "actions":
                category = "actions"

            entries = self._collect_category(self.root / dir_name, category)
            catalog.infra_files.extend(entries)
            catalog.entries.extend(entries)
            for entry in entries:
                catalog.dir_summary[entry.source_dir] = catalog.dir_summary.get(entry.source_dir, 0) + 1
            if dir_name == "dbs":
                catalog.dbs_files.extend(entries)
            if dir_name == "actions":
                catalog.action_files.extend(entries)

        for name in sorted(set(ROOT_PYTHON_WHITELIST)):
            path = self.root / name
            if path.is_file():
                entry = DirectiveEntry(
                    path=path,
                    category="root_python",
                    source_dir="root",
                    kind=_classify_kind(path),
                )
                catalog.root_python_files.append(entry)
    ...
    def scan_dict(self) -> Dict[str, Any]:
        ...
        return {
            "total": catalog.total,
            "dbs_files": [entry.as_dict() for entry in catalog.dbs_files],
            "action_files": [entry.as_dict() for entry in catalog.action_files],
            "finance_files": [entry.as_dict() for entry in catalog.finance_files],
            "math_files": [entry.as_dict() for entry in catalog.math_files],
            "infra_files": [entry.as_dict() for entry in catalog.infra_files],
            "root_python_files": [entry.as_dict() for entry in catalog.root_python_files],
            "dir_summary": dict(catalog.dir_summary),
        }
